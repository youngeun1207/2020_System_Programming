...
int remainderPower2(int x, int n) {
    ...
    int signMask = x>>31<<n;
    int not0Mask = !remainder+one;
    return (signMask & not0Mask)|remainder;
}
/* 
...
int satMul2(int x) {
  int mul2 = x<<1;
  int overflowMask = (mul2^x)>>31;
  int tmin = 1<<31;
  int mul2isNeg = mul2 >>31;
  return (~overflowMask&mul2)|(overflowMask&(tmin^(mul2isNeg))); 
}
/* 
...
int signMag2TwosComp(int x) {
  int mask = x>>31;
  int sign = mask <<31;
  int withoutSign = x^sign;
  return (withoutSign^mask)+(mask&1);
}
